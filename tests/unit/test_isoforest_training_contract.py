...
def _train_isoforest():
    return import_module('ml.training.train_isoforest')
...
def test_train_isoforest_writes_artifacts_and_evaluate_split_metrics(tmp_path):
    train_isoforest = _train_isoforest()
    train = _write_skab_csv(tmp_path / 'train.csv', offset=0.0)
    ...
    result = train_isoforest.train_isoforest_from_skab(
    ...
    assert metadata['model_family'] == 'isolation_forest'
    ...
    assert metadata['params']['n_estimators'] == 8
    assert metadata['params']['contamination'] == 'auto'
