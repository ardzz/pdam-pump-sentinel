...
def _enriched():
    return import_module('ml.features.enriched')
...
def test_build_enriched_window_features_shape_and_domain_semantics():
    pd = _pandas()
    enriched = _enriched()
    loader = _skab_loader()
    sensor_columns = loader.SENSOR_COLUMNS
    frame = pd.DataFrame(
        {
            ...
            **{
                column: [float(row_index + column_index) for row_index in range(5)]
    ...
    features, labels, changepoints, timestamps = enriched.build_enriched_window_features(
        frame,
        window_size=3,
        stride=2,
    ...
    spectral_features, *_ = _spectral().build_spectral_window_features(
        frame,
        window_size=3,
        stride=2,
        sensor_columns=sensor_columns,
    ...
    spectral_count = len(sensor_columns) * (2 + 4)
    assert features.shape == (2, spectral_count + (len(sensor_columns) * 3) + 6)
    assert labels.tolist() == [1, 0]
    assert changepoints.tolist() == [1, 0]
    assert timestamps.tolist() == ['2024-01-01T00:00:02Z', '2024-01-01T00:00:04Z']
    np.testing.assert_allclose(features[:, :spectral_count], spectral_features)
    np.testing.assert_allclose(features[0, spectral_count : spectral_count + len(sensor_columns)], np.arange(8.0))
    np.testing.assert_allclose(
        features[0, spectral_count + len(sensor_columns) : spectral_count + 16], np.arange(2.0, 10.0)
    )
    np.testing.assert_allclose(features[0, spectral_count + 16 : spectral_count + 24], np.full(8, 2.0))
    np.testing.assert_allclose(
        features[0, -6:],
        [-1.0, 5.0 / (6.0 + 1e-12), 32.0, 4.0 / (8.0 + 1e-12), 21.0, 7.0 / (3.0 + 1e-12)],
    )


def test_enriched_training_fits_and_calibrates_without_split_leakage(tmp_path, monkeypatch):
    ...
    assert fit_calls[0].shape == (4, 94)
    assert len(calibrate_calls) == 1
    assert calibrate_calls[0].shape == (2, 94)
    assert result.params['feature_mode'] == 'enriched'
    assert result.params['spectral_n_bands'] == 4


def test_enriched_training_path_returns_evaluate_split_metric_keys(tmp_path):
    train_pca = _train_pca()
    train = _write_skab_csv(tmp_path / 'train.csv', row_count=16, offset=0.0)
    validation = _write_skab_csv(
        tmp_path / 'val.csv', row_count=16, anomaly_start=12, offset=100.0, changepoint_indices=[11]
    )
    test = _write_skab_csv(
        tmp_path / 'test.csv', row_count=16, anomaly_start=12, offset=200.0, changepoint_indices=[12]
    )
